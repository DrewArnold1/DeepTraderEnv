...
if __name__ == '__main__':
    ...
    env.seed(0)
    np.random.seed(0)
    params = dict(n_iter=10000, batch_size=1, elite_frac = 0.2)
    num_steps = 200
    ...
    for (i, iterdata) in enumerate(
        ...
        print('Iteration %2i. Episode reward: %7.3f'%(i, np.sum(iterdata['ys'])))
        agent = BinaryActionLinearPolicy(iterdata['theta_mean'])
        if args.display: do_rollout(agent, env, 200, render=False)
